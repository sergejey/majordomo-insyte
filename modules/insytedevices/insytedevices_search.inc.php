...
*/
 global $session;
  ...
  global $save_qry;
  if ($save_qry) {
   $qry=$session->data['insytedevices_qry'];
  } else {
   $session->data['insytedevices_qry']=$qry;
  }
  if (!$qry) $qry="1";
  $sortby_insytedevices="ADDRESS";
  $out['SORTBY']=$sortby_insytedevices;
  // SEARCH RESULTS
  $res=SQLSelect("SELECT * FROM insytedevices WHERE $qry ORDER BY ".$sortby_insytedevices);
  if ($res[0]['ID']) {
   ...
   for($i=0;$i<$total;$i++) {
    ...
    $data=SQLSelect("SELECT * FROM insytecommands WHERE DEVICE_ID=".$res[$i]['ID']);
    $max_update=0;
    foreach($data as $k=>$v) {
